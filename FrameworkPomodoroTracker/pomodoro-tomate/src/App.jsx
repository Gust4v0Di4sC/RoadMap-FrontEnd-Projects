...
function App() {
  ...
  return (
    <div
      className={`min-h-screen transition-colors duration-500 ease-in-out ${getThemeColors()} font-sans flex flex-col items-center justify-center p-4 relative overflow-hidden`}
    >
      ...
      <main className="w-full max-w-md bg-white/10 backdrop-blur-lg rounded-3xl p-8 shadow-2xl border border-white/20 z-10">
        {/* Cabeçalho */}
        <Header handleClick={() => setIsSettingsOpen(true)} />

        {/* Seleção de Modo (Tabs) */}
        <nav
          className="flex justify-center gap-2 mb-8 bg-black/10 p-1 rounded-full"
          role="tablist"
        >
          {[
        ...
        <div className="text-center mb-8 relative">
          <div
            className="text-[6rem] leading-none font-bold tracking-tighter drop-shadow-lg font-mono"
            role="timer"
            aria-live="polite"
          >
            {formatTime(timeLeft)}
        ...
        <div className="flex justify-center items-center gap-6 mb-8">
          <button
            onClick={toggleTimer}
            className={`h-24 w-24 rounded-3xl flex items-center justify-center shadow-lg transform transition-all active:scale-95 focus:outline-none focus:ring-4 focus:ring-white/50 ${
              isActive
          ...
          >
            {isActive ? (
              <Pause size={40} className="text-red-600" fill="currentColor" />
            ) : (
        ...
        {/* Rodapé / Status */}
        <Footer cycles={cycles} config={config} />
      </main>
      ...
      {isSettingsOpen && (
        <ConfigModal
          icones={{
            principal: Settings, // Ícone obrigatório
            acao: X, // Ícone opcional (será renderizado)
          }}
          handleClick={() => setIsSettingsOpen(false)}
          Icone={X}
          config={config}
